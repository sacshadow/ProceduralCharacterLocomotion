...
public class FS_JingSheTui : FightStyleBase {
	...
	public HAC_FeiTi feiTi;
	public HAC_SaoTangTui saoTangTui;
	public HA_Step step;
	
	
	// public HAC_YingYangZhang_Yin yinZhang;
	
	// public HAC_RFSB rfsb;
	// public HAC_PianNing pianNing;
	
	
	public FS_JingSheTui() {
		...
		saoTangTui = new HAC_SaoTangTui();
		step = new HA_Step();
		
		movement = MoveStyleBase.Default();
		
		// yinZhang = new HAC_YingYangZhang_Yin();
	...
	public override IEnumerator FightStanceB() {
		...
		while(control.Legal(input.action_1.IsPress())) {
			UpdateInput();
			
			if(input.action_0.IsDown()) {
				yield return control.StartCoroutine(control.DoStateLegal(feiTi));
				break;
			}
			if(input.jump.IsDown())
				yield return control.StartCoroutine(control.DoStateLegal(saoTangTui));
			if(input.run.IsDown())
				yield return control.StartCoroutine(control.DoStateLegal(step));
